            ...
            for phone_number in phone_numbers:
                ...
                for card in final_sms_cards:
                    ...
                    if sms_text_p:
                        ...
                        date_str = get_utc_now().strftime('%Y-%m-%d %H:%M:%S')
...
async def check_sms_job(context: ContextTypes.DEFAULT_TYPE):
    ...
    print(f"\n--- [{now_str}] Checking for new messages ---")
...
def main():
    # 1. Start the dummy Flask server in a background thread
    flask_thread = threading.Thread(target=run_flask, daemon=True)
    flask_thread.start()
    print("🌐 Fake Flask server started for Render port binding.")
    ...
    # 4. Schedule the repeating job
    job_queue = application.job_queue
    # Waits 10 seconds before the first run, then repeats every POLLING_INTERVAL_SECONDS
    job_queue.run_repeating(check_sms_job, interval=POLLING_INTERVAL_SECONDS, first=10)
